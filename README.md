...
```bash
python setup.py        # first-run wizard: asks the mandatory parameters
                       # (provider, API key, auth token) and writes .env
                       # immediately — nothing is left to configure by hand
docker compose up -d
```

Prefer files over prompts? `cp .env.example .env` and edit — keyless by
default (`LLM_PROVIDER=none`), no API key required.
