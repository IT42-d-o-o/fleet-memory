...
# Mandatory params are asked for, not assumed: an interactive run prompts for
# anything missing; a non-interactive run fails fast with instructions.
if [ "$LLM_PROVIDER" = "openai" ] && [ -z "${OPENAI_API_KEY:-}" ]; then
    if [ -t 0 ]; then
        read -r -s -p "OpenAI API key (mandatory for LLM_PROVIDER=openai): " OPENAI_API_KEY; echo
        [ -n "$OPENAI_API_KEY" ] || { echo "error: key is mandatory" >&2; exit 1; }
        export OPENAI_API_KEY
    else
        echo "error: OPENAI_API_KEY not set and no TTY to ask - export it or use LLM_PROVIDER=ollama" >&2
        exit 1
    fi
fi
...
echo "Miner supports:"
echo "  (default)             Claude/Codex/Cursor/Gemini transcripts — auto-discovered"
echo "  --git-roots <dir>     Scan subdirs for git repos"
echo "  --markdown-roots <d>  Scan subdirs for markdown docs"
echo "  --github              Mine GitHub issues + commits (set GITHUB_TOKEN)"
echo "  --gitlab              Mine GitLab issues + commits (set GITLAB_TOKEN)"
...
EOF
echo ""
echo "=== Agent behavioral instructions ==="
echo "Paste this into CLAUDE.md (Claude Code), AGENTS.md (Codex), .cursor/rules (Cursor),"
echo "or wherever your AI tool accepts a persistent system prompt:"
echo ""
cat "$REPO_DIR/onboarding/agent-instructions.md"
echo ""
echo "=== Done. ==="
