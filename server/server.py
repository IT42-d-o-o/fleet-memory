...
from fts_index import FtsIndex, rrf_merge, admit_keyword_hits, FUSION_STRATEGIES
...
# Hybrid fusion strategy — one of fts_index.FUSION_STRATEGIES (see the fusion
# strategies block there). An unknown value must not silently masquerade as an
# active strategy: warn loudly and fall back to classic.
FUSION_STRATEGY = os.environ.get("MEM0_FUSION_STRATEGY", "classic")
if FUSION_STRATEGY not in FUSION_STRATEGIES:
    log.warning(
        "unknown MEM0_FUSION_STRATEGY %r (valid: %s) — falling back to 'classic'",
        FUSION_STRATEGY, ", ".join(FUSION_STRATEGIES),
    )
    FUSION_STRATEGY = "classic"
# Dark query capture for the shadow eval: JSONL file path, off when unset.
QUERY_CAPTURE = os.environ.get("MEM0_QUERY_CAPTURE", "")
...
fts = FtsIndex(FTS_DB)
log.info("fts side index ready at %s (%d rows) fusion_strategy=%s",
         FTS_DB, fts.count(), FUSION_STRATEGY)
...
def _resolve_supersession(hits: list[dict], include_superseded: bool) -> list[dict]:
    ...
    for hit in hits:
        ...
        hit_id = hit.get("id")
        if hit_id and "current" not in meta and "superseded_by" not in meta:
            fetched = _fetch_record(hit_id)
            if fetched:
...
def search_memory(query: str, limit: int = 5, project: str | None = None,
    ...
    # Admission counters: how many keyword hits actually vote under the active
    # strategy (for per_hit that is the per-hit identifier admission).
    kw_admitted = (len(admit_keyword_hits(query, keyword))
                   if FUSION_STRATEGY == "per_hit" else len(keyword))
    merged = rrf_merge(semantic, keyword, fetch_n, strategy=FUSION_STRATEGY, query=query)
    ...
    log.info(
        "search ns=%s vec_ms=%s fts_ms=%.0f resolve_ms=%.0f total_ms=%.0f hits=%d kw=%d/%d",
        namespaces, [round(v) for v in vec_ms],
        (t2 - t1) * 1000, (t3 - t2) * 1000, (t3 - t0) * 1000, len(resolved[:limit]),
        kw_admitted, len(keyword),
    )

    # Optional dark query capture for the shadow eval: JSONL, off unless
    # MEM0_QUERY_CAPTURE names a file. Best-effort — never breaks search.
    if QUERY_CAPTURE:
        try:
            with open(QUERY_CAPTURE, "a", encoding="utf-8") as fh:
                fh.write(json.dumps({
                    "ts": datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
                    "query": query,
                    "namespaces": namespaces,
                    "top5_ids": [r.get("id") for r in resolved[:limit]],
                }, default=str) + "\n")
        except Exception:  # noqa: BLE001 — capture is observability, not correctness
            pass
