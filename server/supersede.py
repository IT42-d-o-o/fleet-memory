...
Vault token: file named by VAULT_TOKEN_FILE (default /etc/memory-mcp/vault-token)
"""
import os
import sys
import json
...
def vault_get(path, field):
    r = subprocess.run(
        ...
        env={**os.environ, "VAULT_ADDR": os.environ.get("VAULT_ADDR", "http://127.0.0.1:8200"),
             "VAULT_TOKEN": open(os.environ.get("VAULT_TOKEN_FILE", "/etc/memory-mcp/vault-token")).read().strip()}
    )
    if r.returncode != 0:
...
from qdrant_client.models import Record  # noqa: E402
...
def scroll_all(client: QdrantClient) -> list[Record]:
    all_points = []
    offset = None
    while True:
...
def extract_records(all_points: list[Record]) -> list[dict]:
    records = []
...
def _run(
    ...
    client = QdrantClient(host="127.0.0.1", port=6333, check_compatibility=False, timeout=120)
