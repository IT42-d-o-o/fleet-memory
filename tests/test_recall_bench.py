...
def test_extract_keywords_empty_and_none_safe():
    ...
    assert extract_keywords(None) == set()  # type: ignore[arg-type] — None-safety is the point
...
def test_is_hit_any_of_semantics():
    text = "the address is 192.0.2.138"
    assert is_hit(text, ["CT356", "192.0.2.138"]) is True  # only 2nd matches
...
def test_is_hit_empty_expect_any_is_false():
    ...
    assert is_hit("anything", None) is False  # type: ignore[arg-type] — None-safety is the point
    print("is_hit empty expect_any safety OK")


def test_is_hit_empty_text_is_false():
    assert is_hit("", ["x"]) is False
    assert is_hit(None, ["x"]) is False  # type: ignore[arg-type] — None-safety is the point
